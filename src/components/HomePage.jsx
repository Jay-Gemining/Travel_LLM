...
const HomePage = ({ setIsLoading, setError, setPlan }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // 将必游景点字符串转换为数组，并过滤掉空字符串
    const mustVisitPoisArray = mustVisitPois.split(/,|，/).map(poi => poi.trim()).filter(poi => poi);
    ...
    try {
      const response = await axios.post('/api/plan', {
        ...
        must_visit_pois: mustVisitPoisArray
      });
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-400 via-indigo-500 to-purple-600 flex flex-col justify-center items-center p-4">
      <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label htmlFor="mustVisitPois" className="block text-sm font-medium text-gray-700 mb-1">
              必游景点 (可选，用逗号分隔)
            </label>
            <input
              type="text"
              id="mustVisitPois"
              value={mustVisitPois}
              onChange={(e) => setMustVisitPois(e.target.value)}
